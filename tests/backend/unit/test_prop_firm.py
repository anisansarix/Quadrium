...
def test_prop_firm_evaluate_passed():
    profile = PropFirmProfile(
        ...
        phases=[ChallengePhase(name="P1", profit_target_pct=Decimal('0.10'), min_trading_days=2)]
    )
    
    trades = [
        Trade("1", "XAUUSD", "long", datetime(2023, 1, 1), datetime(2023, 1, 1), Decimal('1'), Decimal('2'), Decimal('1'), Decimal('600')),
        Trade("2", "XAUUSD", "long", datetime(2023, 1, 2), datetime(2023, 1, 2), Decimal('1'), Decimal('2'), Decimal('1'), Decimal('500'))
    ]
    # No drawdown
    equity = np.array([10000, 10600, 11100])
    
    res = PropFirmSimulator.evaluate(profile, trades, Decimal('10000'), equity)
...
def test_prop_firm_evaluate_overall_dd_breach():
    ...
    trades = [
        Trade("1", "XAUUSD", "long", datetime(2023, 1, 1), datetime(2023, 1, 1), Decimal('1'), Decimal('2'), Decimal('1'), Decimal('-1500'))
    ]
...
def test_prop_firm_evaluate_daily_loss_breach():
    profile = PropFirmProfile(
        ...
        max_overall_drawdown_pct=Decimal('0.10'),
        drawdown_type=DrawdownType.STATIC,
        max_daily_loss_pct=Decimal('0.05'), # Max $500 loss per day on $10k
        daily_loss_includes_floating=True,
        daily_reset_timezone="UTC"
    )
...
def test_prop_firm_evaluate_consistency_breach():
    profile = PropFirmProfile(
        ...
        daily_loss_includes_floating=True,
        daily_reset_timezone="UTC",
        consistency_rules=[ConsistencyRule(ConsistencyType.MAX_DAY_SHARE, Decimal('0.30'))] # Max 30% per day
    )
    trades = [
        # Total profit = 1000. Day 1 = 800 (80%), Day 2 = 200 (20%)
        Trade("1", "XAUUSD", "long", datetime(2023, 1, 1), datetime(2023, 1, 1), Decimal('1'), Decimal('2'), Decimal('1'), Decimal('800')),
        Trade("2", "XAUUSD", "long", datetime(2023, 1, 2), datetime(2023, 1, 2), Decimal('1'), Decimal('2'), Decimal('1'), Decimal('200'))
    ]
...
def test_prop_firm_evaluate_min_days():
    profile = PropFirmProfile(
        ...
        daily_loss_includes_floating=True,
        daily_reset_timezone="UTC",
        phases=[ChallengePhase(name="P1", profit_target_pct=Decimal('0.10'), min_trading_days=5)]
    )
    trades = [
        Trade("1", "XAUUSD", "long", datetime(2023, 1, 1), datetime(2023, 1, 1), Decimal('1'), Decimal('2'), Decimal('1'), Decimal('1500'))
    ]
...
def test_prop_firm_empty_trades():
    profile = PropFirmProfile("Test", Decimal('10000'), Decimal('0.1'), DrawdownType.STATIC, Decimal('0.05'), True, "UTC")
    res = PropFirmSimulator.evaluate(profile, [], Decimal('10000'), np.array([]))
    assert res["status"] == ChallengeResult.IN_PROGRESS.value
